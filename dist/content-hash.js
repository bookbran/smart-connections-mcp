 ...
 */
export function canonicalizeMarkdown(raw) {
    let text = raw;
...
export function hashCanonicalText(canonical) {
 ...
 */
export function canonicalContentHash(raw) {
 ...
 * fine."
 */
export function fingerprintMatches(stored, currentHash) {
    if (!stored)
        return false;
    if (stored.contentHashAlgorithm !== CONTENT_HASH_ALGORITHM)
        return false;
