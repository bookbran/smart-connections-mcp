...
export function buildLinkGraph(vaultPath) {
    ...
    for (const rel of rels) {
        ...
        for (const m of stripped.matchAll(LINK)) {
            ...
            if (!target) {
                if (NOT_NOTES.has(key))
                    continue;
                if (!unresolved.has(raw))
                    unresolved.set(raw, new Set());
                unresolved.get(raw).add(rel);
                continue;
            }
            if (target === rel)
                continue; // self-links add nothing
            if (!edges.has(rel))
 ...
 */
export function resolveLink(graph, link) {
 ...
 * stray link is not.
 */
export function integrityReport(graph, minRefs = 3) {
