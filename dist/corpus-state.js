...
export function corpusIsClean(state) {
    return (state.errors.inventory.length === 0 &&
...
export class CorpusReconciler {
    ...
    recordEmbedFailure(path, message) {
        this.embedErrors.set(path, { scope: 'embed', path, message });
    }
    clearEmbedFailure(path) {
    ...
    async doReconcile() {
        ...
        await mapWithConcurrency(inventory.values(), READ_CONCURRENCY, async (entry) => {
            ...
            if (rawKey !== undefined) {
                ...
                if (verdict === 'verified-current')
                    state.pluginFresh.add(entry.path);
                else
                    state.pluginStale.add(entry.path);
            }
...
//# sourceMappingURL=corpus-state.js.map