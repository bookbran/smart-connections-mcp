...
import { CorpusReconciler, type CorpusState } from './corpus-state.js';
import {
  buildSupplementalIndex,
  DEFAULT_EMBED_BUDGET,
  type SupplementalIndex,
...
export class CorpusProvider {
  private readonly reconciler: CorpusReconciler;
  ...
  async snapshot(): Promise<CorpusState> {
  ...
  async get(options: CorpusOptions = {}): Promise<CurrentCorpus> {
    ...
    let supplemental: SupplementalIndex;
  ...
  private assemble(state: CorpusState, supplemental: SupplementalIndex): CurrentCorpus {
    const noteVectors = new Map<string, number[]>();
    const sectionVectors = new Map<string, BlockVector[]>();
    ...
    for (const [rawPath, source] of this.loader.getSources()) {
      ...
      if (!path || !state.pluginFresh.has(path)) continue;
      const vec = source.embeddings?.[modelKey]?.vec;
      if (vec?.length) noteVectors.set(path, vec);
      const headings = Object.keys(source.blocks || {});
      if (headings.length) blockHeadings.set(path, headings);
    }
    ...
    for (const [rawPath, blocks] of this.loader.getBlockVectors()) {
      ...
      if (usable.length) sectionVectors.set(path, usable);
    }
    ...
    const semanticSearchable = new Set<string>();
    for (const path of noteVectors.keys()) semanticSearchable.add(path);
    for (const path of sectionVectors.keys()) semanticSearchable.add(path);
    ...
    for (const path of Array.from(semanticSearchable)) {
      if (!state.onDisk.has(path)) semanticSearchable.delete(path);
    }
