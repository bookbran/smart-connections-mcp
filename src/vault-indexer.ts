...
import {
  CHUNKER_VERSION,
  loadSupplementalCache,
  saveSupplementalCache,
  type SupplementalCache,
} from './supplemental-store.js';
...
export function emptySupplementalIndex(): SupplementalIndex {
  return {
    vectors: new Map(),
    sections: new Map(),
    attempted: 0,
...
export async function buildSupplementalIndex(
  ...
  for (const path of state.supplementalFresh) {
    ...
    if (entry.sections.length) result.sections.set(path, entry.sections);
  ...
  for (const path of pending) {
    if (result.embedCalls >= maxEmbeddings) break;
    reached++;
    result.attempted++;
    ...
    if (outcome.status === 'failed') {
      ...
      if (outcome.fatal) break; // model is gone; the rest of this run is pointless
      continue;
    }
    ...
    if (outcome.sections.length) result.sections.set(path, outcome.sections);
  }

  result.remaining = Math.max(0, pending.length - reached);
  if (cacheDirty) saveSupplementalCache(vaultPath, cache);
  return result;
}

type EmbedOutcome =
  | { status: 'current'; vec: number[]; sections: number[][]; embedCalls: number }
  | { status: 'raced'; embedCalls: number }
  | { status: 'failed'; message: string; fatal: boolean; embedCalls: number };
...
async function embedOne(
  ...
  for (const section of splitIntoSections(canonicalKey, canonical)) {
    if (embedCalls >= remainingBudget && sections.length > 0) break;
    let sv: number[];
    try {
